...
while (<FILIN>) {
  ...
  if ($_ =~ /^\w/) {
    $prefix = $_;
    $nfile++;
    $fileout = "$prefix.opls.mdlog";
    $file[$nfile] = "$fileout";
  }
}

open(FILOUT,">$summary");
print FILOUT "               Test        TotalE      BondE      AngleE       TorsionE     ImpTorE        vdwE         ElecE      vdW14E       Elece14E        WaterE\n";
for ($i = 1; $i <= $nfile; ++$i) {
  open(FILIN,"<$file[$i]");
  while(<FILIN>) {
    #    if ($_ =~ /\-/) {$_ =~ s/\-/ \-/g;}
  ...
  close(FILIN);
  printf FILOUT "%20s %12.6f %12.6f %12.6f %12.6f %12.6f %14.6f %14.6f %12.6f %12.6f %12.6f\n",$file[$i],$total,$bond,$angle,$torsion,$improper,$vdw,$elec,$vdw14,$elec14,$water;
}
close(FILOUT);
