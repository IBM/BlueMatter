...
CreateSetDialog::CreateSetDialog(TracerWindow *Parent)
{
   ...
   char   *Initial_Filter = (char *) "*";
   char   *Initial_Prev_Filter = (char *) "*Start*";
   char   *Initial_Post_Filter = (char *) "*Finis*";
   LastOffset = 0;
   char SepStr[12];
   ...
   if (strcmp(Parent->StartDirectory,"") == 0)
   {
      ...
      if (getcwd(CurrentDirectory,MAX_FILENAME) == NULL)
      {
          ...
          sDir = XmStringCreateLocalized((char *) "~");
      }
   ...
   s = XmStringCreateLocalized((char *) "Create Trace Set");
   ...
   str = XmStringCreateSimple( (char *) "Set Name:");
   ...
   str = XmStringCreateSimple((char *) "Trace Separation:");
   ...
   str = XmStringCreateSimple((char *) "Offset:");
   ...
   str = XmStringCreateSimple((char *) "Select ALL Traces");
   ...
   CreatePreviousForm(wExtraForm,(char *)  "*Start*");
   CreatePostForm(wExtraForm, (char *)  "*Finis*");
...
void CreateSetDialog::CreatePreviousForm(Widget Parent, char* Initial_Filter)
{
   ...
   liPrev = XmCreateScrolledList(fmPrev,(char *) "liPrev",args,n);
...
void CreateSetDialog::CreatePostForm(Widget Parent, char* Initial_Filter)
{
   ...
   liPost = XmCreateScrolledList(fmPost,(char *) "liPost",args,n);
    ...
    if (w ==  wCreSetPrevTF)
    {
      ...
      for (loop =0; loop + 5 < strlen(Filter); loop++)
      {
            ...
            (Filter[loop + 4] == 't') )
        {
           ...
           NewPostFilter[loop] = 0 ;
           strcat(NewPostFilter,"Finis");
           NewPostFilter[loop+5] = 0 ;
...
Boolean CreateSetDialog::LoadGroupFromList()
{
  ...
  {
     MessageBox = new MessageDialog(wCreSetDialog,(char *) "No file selected",ERROR);
     return FALSE;
  }
  ...
  if (GraphStyle != POINTS_ONLY_STYLE)
  {
               ...
               XmTextGetString(wCreSetPostTF)) == 0)
    {
      ...
      MessageBox = new MessageDialog(wCreSetDialog,
          (char *) "Regular Expression Text fields contain the same data",
                         ERROR);
    ...
    if (NumPreviousFileListItems != NumPostFileListItems)
    {
      ...
      MessageBox = new MessageDialog(wCreSetDialog,
          (char *) "Number of file is different",ERROR);
  ...
  try
    {
      ...
      Group->Initialize( ParentWindow, chGroupName, (char *) " No_file_name",
