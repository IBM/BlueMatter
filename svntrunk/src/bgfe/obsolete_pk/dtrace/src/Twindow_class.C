...
//  CurrentRightTime = (MidTime - 1) + (WindowSpan / 2 );
  ...
  ConstructGC( &formGC, form0, 1 ,
               "yellow", "lightslategrey", "6x10", NULL , NULL);

  ConstructGC( &timeGC, form0, 1 ,
               "green", "lightslategrey", "8x13", NULL , NULL);

 
  ConstructGC( &midGC, da, 1 , "yellow", NULL, NULL, NULL , NULL);
...
//   CurrentRightTime = (MidTime - 1) + (WindowSpan / 2 );
   ...
   XDefineCursor(XtDisplay(GetApplicationShell()),
                 ...
                 Mouse_cursor);
   GetApplication()->SendMessageText("\nDrawing...");
