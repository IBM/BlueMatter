...
ScaleDialog::ScaleDialog(TracerWindow *ParentWindow)
{
   ...
   wScaleDialog = XmCreateFormDialog( Parent->form0, (char *)  "Scale", args, n );
...
void ScaleDialog::guiDrawTimeScale( )
{
   ...
   mu_TimeScale = XmCreatePulldownMenu( rc_Menu, (char *) "menu", args, 0 );


   str = XmStringCreateSimple((char *) "Time Scale");

   n = 0;
   XtSetArg( args[n],XmNsubMenuId, mu_TimeScale ); n++;
   XtSetArg( args[n],XmNlabelString, str ); n++;

   mu_Option = XmCreateOptionMenu( rc_Menu,(char *)  "option", args, n );
