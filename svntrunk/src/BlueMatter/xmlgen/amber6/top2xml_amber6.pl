...
# revised JWP
# 29 March 2005
# now reads all entries in fixed format as specified in the prmtop spec
...
# now grab the rest of the file as one big list
# "@input_list"
# 
# WARNING: whitespace-delimiting may not be OK for files
# with many (>99999) atoms. . .
#

print STDERR "read title, now getting input string\n";
undef $/;
$_=<INPUT>;
$temp_length = length($_);
$input_string = $_;
print STDERR "chomping input string\n";
chomp $input_string;
$input_pointer = 0;
$input_length = length($input_string);
print STDERR "temp_length $temp_length input_length $input_length\n";
...
# array name
#
# we also need the @param_fmt_list -- number of characters
# in each element of the array for that parameter
# used by the read_data call to do fixed format reads
# (we no longer split $_ to get the input_list array)
# this of course makes the %param_fmt_hash. . .
...
# these are in i6 format

$topo_params=0;

print STDERR "reading topo params from top of file\n";

while ($topo_params < 30) {
	print STDERR "reading $topo_params\n";
	$val = read_data(6);
	print STDERR " ptr: $input_pointer val: #$val#\n";
	%tmp = ($param_key[$topo_params] => $val);
	%param_hash = (%param_hash, %tmp);
	$topo_params++;
}
print STDERR "done reading topo params \n";
...
%param_sz_hash =();
$data_needed = 0;
print STDER "making param_sz_hash\n";
if ($#param_vars == $#param_sz_list) {
	...
	foreach $param (@param_vars) {
		...
		%param_sz_hash = (%param_sz_hash, 
			  $param=>$param_hash{$size});
	}
...
print STDERR "making param_fmt_hash\n";
if ($#param_vars == $#param_fmt_list) {
	foreach $param (@param_vars) {
		$fmt = shift @param_fmt_list;
		$param_fmt_hash{$param}=$fmt;
		$chars_needed += $fmt*$param_sz_hash{$param};
	}
} else {
	print STDERR "Parameter format array problems\n";
	exit;
}
...
#
# note that this isn't always a hard limit since we don't know how much
# submolecule information is included for periodic boundary conditions
#
#if (($data_needed>$#input_list)||($IFBOX!=1&&$data_needed!=$#input_list)) {
#	print STDERR "ERROR: input file has less data than needed.\n";
#	exit(1);
#}

#
# now read most of the remaining parameters from the list
#

foreach $var (@param_vars) {
	@$var = ();
	$size = $param_sz_hash{$var};
	$fmt = $param_fmt_hash{$var};
	print STDOUT "Reading $size elements of $var with format $fmt\n";
	print STDERR "Reading $size elements of $var with format $fmt\n";
        $index = 1;
	while ($index <= $size) {
		if (($index%1000)==0) {
			print STDERR "  $var $index\n";
		}
		$var->[$index] = read_data($fmt);
		$index++;
	}
	print STDOUT "  $var first element #$var->[1]#\n";
	print STDOUT "  $var last element #$var->[$index-1]#\n";
}
...
#
	    ...
	    print "water anglex: $MOLECULE[$IB[$num_angles]] $num_angles\n";
	  }
	}
	$beta = read_data(16);
	foreach $index (1 .. 3) {
		$BOX[$index] = read_data(16);
	}
	print STDOUT "beta $beta box @BOX\n";
} else {
...
# check to make sure we read everything

if ($#split_list >= 0) {
	print STDERR "$#split_list too many elements in the input prmtop file!";
	exit(1);
}
...
@special_cases = ("CL", "Cl", "NA", "Na","CAL","Ca");
@special_index = (16,16,10,10,19,19);
...
sub read_data {
	#
	# input_string is the master string containing the entire file
	# note that it has carriage returns (\n) in it!
	#
	my $fmt = shift @_;
	if (($input_pointer+$fmt-1) > $input_length) {
		print STDERR "trying to read $fmt from input $input_length\n";
		print STDERR "at $input_pointer\n";
		exit;
	}
	my $str = substr $input_string,$input_pointer,$fmt;
	if ($str =~ /\n/) {
		#print STDERR "found cr in $fmt string #$str#\n";
		#
		# we know the string contains a carriage return
		#
		CHR: foreach $idx (0 .. $fmt-1) {
			$chr = substr $str,$idx,1;
			if ($chr =~ /\n/) {
				#
				# change fmt to only read up to the
				# character before the carriage return
				# our next read will have a cr at the 
				# start of the string and skip ahead. . .
				#
				if ($idx > 0) {
					$fmt = $idx;
					#print STDERR "cr is after $fmt\n";
					last CHR;
				} else {
					#
					# sometimes the CR is at the start
					# of the string, we need to skip
					# ahead
					#
					$input_pointer++;
					#print STDERR "cr is at start\n";
					last CHR;
				}
			}
		}
		$str = substr $input_string, $input_pointer, $fmt;
		#print STDERR "cr new string #$str#\n";
	}
	#my $tmp = substr $input_string,$fmt;
	#$input_string = $tmp;
	$input_pointer += $fmt;
	#
