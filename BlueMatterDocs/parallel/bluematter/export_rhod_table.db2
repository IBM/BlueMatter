--Script to export table with summary performance data for Rhodopsin system for:
-- V4 MPI, group_id = 188
-- V4 SPI, group_id = 222
-- V5 SPI, group_id = 247
export to rhod_table.del of del
 with 
 mpi_mesh (trace_set_id, p_x, p_y, p_z) as (select trace_set_id, int(p_x), int(p_y), int(p_z) from proc_mesh
 ...
 spi_mesh (trace_set_id, p_x, p_y, p_z) as (select trace_set_id, int(p_x), int(p_y), int(p_z) from proc_mesh
  where trace_set_id in (select trace_set_id from perf.trace_set where group_id=222)),
 spi_v5_mesh (trace_set_id, p_x, p_y, p_z) as (select trace_set_id, int(p_x), int(p_y), int(p_z) from proc_mesh
  where trace_set_id in (select trace_set_id from perf.trace_set where group_id=247)),
 ...
 spi_tot (trace_set_id, tot) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='P2_Simtick_Loop_Control' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=222)),
 spi_v5_tot (trace_set_id, tot) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='P2_Simtick_Loop_Control' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=247)),
 ...
 spi_bcast (trace_set_id, bcast) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='ThrobberBcast' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=222)),
 spi_v5_bcast (trace_set_id, bcast) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='ThrobberBcast' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=247)),
 ...
 spi_reduce (trace_set_id, reduce) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='ThrobberReduce' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=222)),
 spi_v5_reduce (trace_set_id, reduce) as
  (select trace_set_id, avg/1.0e6 from perf.performance where
    trace_label='ThrobberReduce' and trace_set_id in (select trace_set_id from perf.trace_set where group_id=247))
 select mpi_mesh.p_x*mpi_mesh.p_y*mpi_mesh.p_z as node_count,
        ...
        dec(round(43222.0/(mpi_mesh.p_x*mpi_mesh.p_y*mpi_mesh.p_z),1),4,1) as atoms_per_node
  from mpi_mesh,
       spi_mesh,
       spi_v5_mesh,
       mpi_tot, 
