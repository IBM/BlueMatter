...
### psf2xml.pl 
###
### usage:   psf2xml.pl <topology file> <parmameter file> <psf file> <option> <Number of additional waters> <replication number> <additional psfs ...> ...
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 
###          # this generates "enkp.charmm.xml"
###
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 0 4
###          # generates enkp.charmm.xml with 4 additional waters
###
###
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 0 4 3
###          # generates 3 instances of (enkp with 4 additional waters)
###
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 0 4 0 ala.psf
###          # generates enkp and ala with 4 additional waters)
###
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 0 4 0 ala.psf pro.ala
###          # generates enkp, ala, pro with 4 additional waters)
###
### example: psf2xml.pl top_all22_prot.inp par_all22_prot.inp enkp.psf 0 0 5 ala.psf pro.psf
...
$debug = 0;
if ($debug) {
  for ($i =1; $i <= $nsite; ++$i) {print "$i $mol[$i] $resnum[$i] $resname[$i] $atname[$i] $atid[$i] $charge[$i] $mass[$i]\n";}
...
do {
  ...
  if ($_ =~ /^!/ || $_ =~ /^\s+/ || $_ =~ /HBOND/ || $_ eq "" || $_ =~ /^NBFIX/) {
    ...
    ++$nnbparm;
    split;
    $nbparmatom[$nnbparm]  = $_[0];
    $nbparmIndex{$nbparmatom[$nnbparm]} = $nnbparm;
    $nbparme[$nnbparm] = $_[2];
    $nbparmr[$nnbparm] = $_[3];
    if ($_[4] =~ /^\d/) {
      $nb14parme[$nnbparm] = $_[5];
      $nb14parmr[$nnbparm] = $_[6];
    } else {
      $nb14parme[$nnbparm] = $_[2];
      $nb14parmr[$nnbparm] = $_[3];
    }

    if($nnbparm < 20){
    print "  nnbparm $nnbparm  nbparmatom[$nnbparm]  = $nbparmatom[$nnbparm]\n"; 
    print "  nbparmIndex{$nbparmatom[$nnbparm]} = $nbparmIndex{$nbparmatom[$nnbparm]}\n ";
    print "  nbparme[$nnbparm] = $nbparme[$nnbparm]\n";
    print "  nbparmr[$nnbparm] = $nbparmr[$nnbparm]\n";
  }
# recomment
  }
  if ($nbparm[$nnbparm] eq "OT") {
    $nbparme[$nnbparm]   = -0.15207259450172;
    $nbparmr[$nnbparm]   = 1.76824595565275;
    $nb14parme[$nnbparm] = 0.0;
    $nb14parmr[$nnbparm] = 0.0;
  }
  if ($nbparm[$nnbparm] eq "HT") {
    $nbparme[$nnbparm]   = 0.0;
    $nbparmr[$nnbparm]   = 0.0;
    $nb14parme[$nnbparm] = 0.0;
    $nb14parmr[$nnbparm] = 0.0;
...
#	 if($i > 850 && $i < 900)
#	 {     print " $bondparmtag gives bond index $j\n";}
     if($j != 0 &&  $bondparmk[$j] != 0.0) {
	 ++$bond;
	 $bondk[$bond]   = $bondparmk[$j];
	 $bondr[$bond]   = $bondparmr[$j];
	 $bondat1[$bond] = $bondatom1[$i];
	 $bondat2[$bond] = $bondatom2[$i];
#	 if($bond > 850 && $bond < 900)
#	 {
#	     print" $bond  $bondk[$bond] $bondr[$bond] $bondat1[$bond] $bondat2[$bond]\n";
#	 }
     }
 }
...
torsionlbl: for ($i = 1; $i <= $ntorsion; ++$i) {
  ...
  for ($j =1; $j <= $ntorsionparm; ++$j) {
    $check = 0;
    if ($topatname[$atid[$torsionatom1[$i]]] eq $torsionparm1[$j] && 
	$topatname[$atid[$torsionatom2[$i]]] eq $torsionparm2[$j] && 
	$topatname[$atid[$torsionatom3[$i]]] eq $torsionparm3[$j] &&
	$topatname[$atid[$torsionatom4[$i]]] eq $torsionparm4[$j]) {
      $check = 1;
      $score = 1;
    } elsif ($topatname[$atid[$torsionatom1[$i]]] eq $torsionparm4[$j] &&
	     $topatname[$atid[$torsionatom2[$i]]] eq $torsionparm3[$j] && 
	     $topatname[$atid[$torsionatom3[$i]]] eq $torsionparm2[$j] &&
	     $topatname[$atid[$torsionatom4[$i]]] eq $torsionparm1[$j]) {
      $check = 1;
      $score = 1;
    } elsif ($score != 1 && 
	     ($torsionparm1[$j] eq "X" && 
	     $torsionparm2[$j] eq $topatname[$atid[$torsionatom2[$i]]] && 
	     $torsionparm3[$j] eq $topatname[$atid[$torsionatom3[$i]]] &&
	     $torsionparm4[$j] eq "X")) {
      $check = 1;
    } elsif ($score != 1 &&
	     ($torsionparm1[$j] eq "X" && 
	     $torsionparm2[$j] eq $topatname[$atid[$torsionatom3[$i]]] && 
	     $torsionparm3[$j] eq $topatname[$atid[$torsionatom2[$i]]] &&
	     $torsionparm4[$j] eq "X")) {
      $check = 1;
    }
...
improperlbl: for ($i = 1; $i <= $nimproper; ++$i) {
  for ($j =1; $j <= $nimproperparm; ++$j) {
    if (($topatname[$atid[$improperatom1[$i]]] eq $improperparm1[$j] && 
	 $topatname[$atid[$improperatom2[$i]]] eq $improperparm2[$j] && 
	 $topatname[$atid[$improperatom3[$i]]] eq $improperparm3[$j] &&
	 $topatname[$atid[$improperatom4[$i]]] eq $improperparm4[$j]) ||
	($topatname[$atid[$improperatom1[$i]]] eq $improperparm4[$j] && 
	 $topatname[$atid[$improperatom2[$i]]] eq $improperparm3[$j] && 
	 $topatname[$atid[$improperatom3[$i]]] eq $improperparm2[$j] &&
	 $topatname[$atid[$improperatom4[$i]]] eq $improperparm1[$j])) {
      ++$improper;
      ...
      next improperlbl;
    } elsif (($improperparm1[$j] eq $topatname[$atid[$improperatom1[$i]]] && 
	      $improperparm2[$j] eq "X" && 
	      $improperparm3[$j] eq "X" &&
	      $improperparm4[$j] eq $topatname[$atid[$improperatom4[$i]]]) ||
	     ($improperparm1[$j] eq $topatname[$atid[$improperatom4[$i]]] && 
	      $improperparm2[$j] eq "X" && 
	      $improperparm3[$j] eq "X" &&
	      $improperparm4[$j] eq $topatname[$atid[$improperatom1[$i]]])) {
      ++$improper;
      $improperk[$i]   = $improperparmk[$j];
      $improperd[$i]   = $improperparmd[$j];
      $improperat1[$i] = $improperatom1[$i];
...
for($i = 1; $i <= $nsite; $i++){
	...
	print " When this happened before on the first nonbonded term, there was no blank line after the nonbond keyword in read params. inserting blank line before first term fixes.";
	die;}

    $element[$i] = $topattype[$topatid[$ii]];
    $tmp = $topatname[$topatid[$ii]];
	...
	$lje[$i]   = - $nbparme[$iii];
	$ljr[$i]   = $nbparmr[$iii];
	$lj14e[$i] = - $nb14parme[$iii];
	$lj14r[$i] = $nb14parmr[$iii];
    }
}
...
for($i = 1; $i <= $nmoleculename; $i++){
  ...
  if($debug_vocab)
    {
      print "molecule name/type $moleculename[$i] $moleculetype[$i]\n";
    }
}
...
print XMLOUT "</data_parameter_list>\n";
  ...
  for($ii = 0 ; $ii < $NReplicas ; $ii++)
  {
    ...
    for($i=1; $i <= $ncc; $i++){
      ...
      if($debug_vocab)
	{
	  print "chemical component instance name $cc[$i]\n";
	}
    }
  }
print XMLOUT "</data_list>\n";
...
if ($nmoni) {
  ...
  for($ii = 0 ; $ii < $NReplicas ; $ii++)
	...
	if($debug_vocab)
	  {
	    print "monomer id/instance id $monichain[$i] $moni[$i]\n";
	  }
      }
    }
  print XMLOUT "</data_list>\n";
...
print XMLOUT " </data_parameter_list>\n";
  ...
  for($ii = 0 ; $ii < $NReplicas ; $ii++)
	...
	if($debug_vocab)
	  {
	    print "molecule instance name $molInst2Name{$i} \n";
	  }
      }
    }
print XMLOUT " </data_list>\n";
...
sub checkmoleculetype {
  ...
  if ($_[0] eq 'ALA' || $_[0] eq 'ARG' || $_[0] eq 'ACE' ||
      ...
      $_[0] eq 'LYS' || $_[0] eq 'LYD' || $_[0] eq 'LYR' || $_[0] eq 'MET' || $_[0] eq 'PHE' ||
    ...
    $foo = "dna"
	   ...
	   $_[0] eq 'RNUU' ){
    $foo = "rna"
  } elsif ($_[0] eq 'C12' ||$_[0] eq 'OLEO' || $_[0] eq 'PCGL' || $_[0] eq 'STEA' || $_[0] eq 'DHA' || $_[0] eq 'PALM' || $_[0] eq 'PEGL' || $_[0] eq 'POPC'){
    $foo = "lipid"
  } elsif ($_[0] eq 'CHOL' || $_[0] eq 'SIS'|| $_[0] eq 'SI'|| $_[0] eq 'PEN' ||
           $_[0] eq 'FIX' || $_[0] eq 'ETHA' || $_[0] eq 'ACEM' 
           || $_[0] eq 'MESH' || $_[0] eq 'IMIA' || $_[0] eq 'BUTA' 
           || $_[0] eq 'IBUT' || $_[0] eq 'EMS' || $_[0] eq 'BENZ'
           || $_[0] eq 'MEOH' || $_[0] eq 'ETOH' || $_[0] eq 'INDO'
           || $_[0] eq 'PHEN' || $_[0] eq 'PROM' || $_[0] eq 'PROP'){
    $foo = "other"
  }elsif ($_[0] eq 'UNK' ){ 
    $foo = "ligand"
